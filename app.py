"""
app.py — Entry point cho Hugging Face Spaces.
Nội dung giống gradio_app.py, nhưng demo.launch() dùng cấu hình phù hợp với HF.
"""
...
def _build_scales(s_min: float, s_max: float, s_step: float) -> list:
...
def _build_angles(a_min: int, a_max: int, a_step: int) -> list:
...
with gr.Blocks(title="AI Pattern Detection") as demo:
    gr.Markdown(
        "## 🔍 AI Pattern Detection\n"
        "Phát hiện ký hiệu kỹ thuật trong bản vẽ — **zero-shot**, chỉ cần 1 ảnh mẫu.\n\n"
